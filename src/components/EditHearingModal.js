...
const EditHearingModal = ({ isOpen, onClose, onSave, hearing }) => {
  ...
  const pickDocument = async () => {
    ...
    try {
      const result = await DocumentPicker.getDocumentAsync({
        type: 'application/pdf', copyToCacheDirectory: true, multiple: true,
      });
      if (!result.canceled) {
  ...
  const validate = () => {
    ...
    Object.keys(formState).forEach(key => {
      if (key === 'id' || key === 'description' || key === 'proceedings' || (isAdmin && key === 'clientEmail')) return; // Tornando email opcional na edição
      if (!formState[key]) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // --- ALTERAÇÃO 1: Usamos o endpoint de upload para arquivos "raw" ---
      ...
      for (let i = 0; i < filesToUpload.length; i++) {
        ...
        if (data.secure_url) {
          // --- ALTERAÇÃO 2: A substituição do link não é mais necessária ---
          // O link agora já vem no formato correto
      ...
      setUploadProgress('Atualizando informações...');
